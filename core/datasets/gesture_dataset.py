...
import sys
sys.path.append('/home/liufengqi/research/Gesture/SpeechDrivesTemplates/')

from core.utils.audio_processing import parse_audio_length, crop_pad_audio
from core.datasets.speakers_stat import *
...
class GestureDataset(Dataset):
    ...
    def __getitem__(self, idx):
        if self.split == 'demo':
            ...
            poses_with_score = self.remove_unuesd_kp(poses_with_score)              # remove lower body  137 -> 122
            relative_poses_with_score = self.absolute_to_relative(poses_with_score)  # relative and remove root rode   122 -> 121
            if self.cfg.HIERARCHICAL_POSE:
                relative_poses_with_score = self.global_to_parted(relative_poses_with_score)

            relative_poses = relative_poses_with_score[:, :2, :]                   # torch.Size([64, 2, 121])
            poses_score = relative_poses_with_score[:, 2:, :].repeat(1, 2, 1)     # # torch.Size([64, 2, 121])
    ...
    def normalize_poses(self, kp, speaker_stat):
        if isinstance(speaker_stat['mean'], np.ndarray):
            ...
            mean = speaker_stat['mean'].to(kp.device)    # torch.Size([242])
            std = speaker_stat['std'].to(kp.device)

        if mean.dim() == 1:
            mean = mean.reshape(1, 2, self.cfg.NUM_LANDMARKS)    # torch.Size([1,2,121])
...
if __name__ == "__main__":
    ...
    dataset = GestureDataset(cfg.DATASET.ROOT_DIR, 'kubinec', 'train', cfg)
    # dataset = GestureDataset(cfg.DATASET.ROOT_DIR, 'kubinec', 'val', cfg)
    ...
    for sample in dataloader:
        ...
        poses_score = sample['poses_score']
        ...
        relative_poses = transform_func(relative_poses)   # torch.Size([1, 64, 2, 121])
